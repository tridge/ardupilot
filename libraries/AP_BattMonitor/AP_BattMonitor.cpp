...
void AP_BattMonitor::read(bool armed)
{
...
/// set_pack_capacity_mah - sets the capacity of the pack, in mAh
void AP_BattMonitor::set_pack_capacity_mah(const int32_t battery_capacity_mah, const uint8_t instance)
{
    if (instance < AP_BATT_MONITOR_MAX_INSTANCES) {
        _params[instance]._pack_capacity.set(battery_capacity_mah);
    } 
}
...
void AP_BattMonitor::display_voltage_only(uint8_t instance, bool display) {
    if (instance >= _num_instances || drivers[instance] == nullptr) {
        return;
    }
    return drivers[instance]->display_voltage_only(display);
}
