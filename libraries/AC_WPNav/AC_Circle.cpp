...
void AC_Circle::calc_velocities(bool init_velocity)
{
    ...
    if (_radius <= 0) {
        ...
        float velocity_max = MIN(_pos_control.get_max_speed_xy_cms(), safe_sqrt(_pos_control.get_max_accel_xy_cmss()*_radius));
...
void AC_Circle::check_param_change()
{
    ...
    if (!is_equal(_last_rate_param,_rate_parm.get())) {
        _rate = _rate_parm;
        _last_rate_param = _rate_parm;
    }
}
