...
void SITL_State::_fdm_input_local(void)
{
    ...
    // Update voltage/current pins
    // {voltage,current}_pin_value is an uint16. Multiply by 1024 to add resolution
    // Voltage divider in spirit ESC board is ~17, Amps per Volt is 50. See Spirit parameters

    voltage_pin_voltage = 1024.0f * _sitl->state.battery_voltage / 17.0f ;
    current_pin_voltage = 1024.0f *  _sitl->state.battery_current / 50.0f ;

    voltage2_pin_voltage = 1024.0f * _sitl->state.battery_voltage / 17.0f ;
    current2_pin_voltage = 1024.0f *  _sitl->state.battery_current / 50.0f ;
