...
public:
    ...
    void get_pfst_fail_status(uint8_t &fail_flag) const {fail_flag = pfst_fail_status;};

    void load_UAS_ID_from_persistent_memory();

    void pilot_declared_emergency_from_gcs(bool emergency) {pilot_declared_emergency = emergency;};
...
private:
    ...
    bool pilot_declared_emergency;
    uint8_t pfst_fail_status = 19; // 0b10011 the 4th bit is only used in the context of the Ascent RID module. Need to init 3rd bit as healthy due to line 277 in cpp
};
