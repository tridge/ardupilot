...
void AP_OpenDroneID::init()
{
    ...
    pkt_system.area_ceiling =           -1000;
    pkt_system.area_floor =             -1000;
    pkt_system.area_count =             1;
    pkt_system.area_radius =            0;
    pkt_system.operator_location_type = 1;
...
bool AP_OpenDroneID::pre_arm_check(char* failmsg, uint8_t failmsg_len)
{
    ...
    // Make this option useless. We should never allow arming without ODID system ready
    ...
    // ASTMF3586 allows arming without a fix as long as hardware is present
    ...
    if (pfst_fail_status){
        strncpy(failmsg, "RemoteID pre-flight safety test failed", failmsg_len);
        return false;
    }
...
void AP_OpenDroneID::send_location_message()
{
    ...
    const bool got_no_gps = (gps_status < AP_GPS::GPS_Status::NO_FIX);
    const bool armed = hal.util->get_soft_armed();

    if (got_no_gps){
        pfst_fail_status |= (1<<1);
    } else {
        pfst_fail_status &= ~(1<<1);
    }
    ...
    uint8_t uav_status;
    if (pilot_declared_emergency){
        uav_status = MAV_ODID_STATUS_EMERGENCY;
    } 
    else {
        uav_status = hal.util->get_soft_armed()? MAV_ODID_STATUS_AIRBORNE : MAV_ODID_STATUS_GROUND;
