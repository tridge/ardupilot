                                 ...
                                 const mavlink_message_t &msg)
{
    ...
    if (msg.compid != NEXT_VISION_COMPID && !routing.check_and_forward(*this, msg)) {
        // the routing code has indicated we should not handle this packet locally
        return;
    }
    if(msg.compid == NEXT_VISION_COMPID && msg.msgid == MAVLINK_MSG_ID_V2_EXTENSION) {
        uint16_t report_type;
        uint8_t buf[256];
        memcpy(&buf, _MAV_PAYLOAD(&msg), msg.len);
        memcpy(&report_type, &buf[0],2);
        if(report_type == 2) {
            gcs().send_to_active_channels(MAVLINK_MSG_ID_V2_EXTENSION, (const char*)&msg.payload64);
            return;
        }
    }
...
{
    if(packet.command == MAV_CMD_DO_SET_CAM_TRIGG_DIST) {
        AP_Mount *mount = AP_Mount::get_singleton();
        if (mount == nullptr){
            return MAV_RESULT_UNSUPPORTED;
        }
        mount->set_trigger_distance(packet.param1);
        if (!isnan(packet.param3)) {
            mount->payload_take_picture();
        }
        return MAV_RESULT_ACCEPTED;
    }
    else if(packet.command == MAV_CMD_IMAGE_START_CAPTURE) {
        AP_Mount *mount = AP_Mount::get_singleton();
        if (mount == nullptr){
            return MAV_RESULT_UNSUPPORTED;
        }
...
void GCS_MAVLINK::handle_odometry(const mavlink_message_t &msg)
{
    ...
    if (!isnan(m.pose_covariance[0])) {
        // Warning: there is a bug in the ModalAI qvio server. Qualcomm API returns the covariance of rotation, then position
        posErr = sqrtf( m.pose_covariance[15] + m.pose_covariance[18] + m.pose_covariance[20]); // In meters
        angErr = sqrtf( m.pose_covariance[0] + m.pose_covariance[6] + m.pose_covariance[11]); // In rad
    }
    ...
    if (!isnan(m.velocity_covariance[0])) {
        velErr = MAX(m.velocity_covariance[0],m.velocity_covariance[6]);
        velErr = MAX(velErr,m.velocity_covariance[11]);
        velErr = sqrtf(velErr); // In m/s.
    }
