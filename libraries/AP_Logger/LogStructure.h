...
struct PACKED log_rid_state {
    LOG_PACKET_HEADER;
    uint64_t time_us;
    uint8_t remote_id_health_flag;
};

struct PACKED log_microhard {
    LOG_PACKET_HEADER;
    uint64_t time_us;
    int frequency;
    int uplinkRssi;
...
LOG_STRUCTURE_FROM_AIS \
    ...
    { LOG_RID_STATE, sizeof(log_rid_state), \
     "RID", "QB","TimeUS,Flag", "s-", "F-" }, \
    { LOG_MICROHARD, sizeof(log_microhard), \
     "MH", "Qiiiiii","TimeUS,Freq,uRSSI,dRSSI,gcsPw,uavPw,SNR", "s------", "F------" }, \
    { LOG_LED_STATE, sizeof(log_led_state), \
     "LED", "QB","TimeUS,State", "s-", "F-" } \
