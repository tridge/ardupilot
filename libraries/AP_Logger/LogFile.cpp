...
/*
    Ascent AeroSystems LogFile Methods
*/
void AP_Logger::write_nv_log(float tilt, float pan, float speedTilt, float speedPan, bool internal_error, bool eo_error, bool ir_error, bool camera_overheat, bool camera_comms_error, float camera_temperature, float trip_temperature)
{
    const struct log_nv_gimbal pkt{
        LOG_PACKET_HEADER_INIT(LOG_NV_GIMBAL),
        time_us                 : AP_HAL::micros64(),
        tilt                    : tilt,
        pan                     : pan,
        speedTilt               : speedTilt,
        ...
        eo_error	            : eo_error,
        ir_error	            : ir_error,
        camera_overheat	        : camera_overheat,
        camera_comms_error	    : camera_comms_error,
        camera_temperature	    : camera_temperature,
        trip_temperature        : trip_temperature

    };
    WriteBlock(&pkt, sizeof(pkt));
}

void AP_Logger::Write_RID_State(uint8_t remote_id_health_flag)
{
    const struct log_rid_state pkt = {
        LOG_PACKET_HEADER_INIT(LOG_RID_STATE),
        time_us      : AP_HAL::micros64(),
        remote_id_health_flag    : remote_id_health_flag
    };
    WriteBlock(&pkt, sizeof(pkt));
}

void AP_Logger::write_mh(int freq, int upRssi, int dnRssi, int gcsPw, int uavPw, int snr)
{
    const struct log_microhard pkt{
        LOG_PACKET_HEADER_INIT(LOG_MICROHARD),
        time_us                 : AP_HAL::micros64(),
        frequency               : freq,
        uplinkRssi              : upRssi,
        downlinkRssi            : dnRssi,
...
void AP_Logger::Write_LED_State(uint8_t led_state)
{
    const struct log_led_state pkt = {
        LOG_PACKET_HEADER_INIT(LOG_LED_STATE),
        time_us      : AP_HAL::micros64(),
        led_state    : led_state
    };
    WriteBlock(&pkt, sizeof(pkt));
...
void AP_Logger::write_vp_log(float pitch_pos, float pan_pos, float pan_pos_relative, float speed_pitch_command, float speed_pan_command)
{
    const struct log_vp_gimbal pkt{
        LOG_PACKET_HEADER_INIT(LOG_VP_GIMBAL),
        time_us     : AP_HAL::micros64(),
        pitch       : pitch_pos,
        pan         : pan_pos,
        pan_rel     : pan_pos_relative,
        speed_pitch : speed_pitch_command,
        speed_pan   : speed_pan_command
    };
    WriteBlock(&pkt, sizeof(pkt));
}

void AP_Logger::write_ga_log(uint8_t geotagging_mode, uint8_t geotagging_session, uint16_t session_pictures, float tilt, uint8_t geotagging_progress, float pan)
{
    const struct log_ga_gimbal pkt{
	        LOG_PACKET_HEADER_INIT(LOG_GA_GIMBAL),
	                time_us                 : AP_HAL::micros64(),
	                geotagging_mode         : geotagging_mode,
	                geotagging_session      : geotagging_session,
	                session_pictures        : session_pictures,
                    tilt                    : tilt,
                    geotagging_progress     : geotagging_progress,
                    pan                     : pan,
	    };
	 WriteBlock(&pkt, sizeof(pkt));
}

void AP_Logger::write_gremsy_log(float pan, float tilt, float roll)
{
    const struct log_gremsy_gimbal pkt{
        LOG_PACKET_HEADER_INIT(LOG_GREMSY_GIMBAL),
            time_us                 : AP_HAL::micros64(),
            pan                     : pan,
            tilt                    : tilt,
            roll                    : roll,
