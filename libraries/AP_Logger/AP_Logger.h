...
public:
    ...
    /*
        Ascent AeroSystems Logging
    */
    void write_vp_log(float pitch, float pan, float pan_relative, float speedTilt, float speedPan);
    void write_nv_log(float tilt, float pan, float speedTilt, float speedPan, bool internal_error, bool eo_error, bool ir_error, bool camera_overheat, bool camera_comms_error, float camera_temperature, float trip_temperature);
    void write_ga_log(uint8_t geotagging_mode, uint8_t geotagging_session, uint16_t session_pictures, float tilt, uint8_t geotagging_progress, float pan);
    void Write_RID_State(uint8_t remote_id_health_flag);
    void Write_LED_State(uint8_t led_state);
    void write_mh(int freq, int upRssi, int dnRssi, int gcsPw, int uavPw, int snr);
    void write_gremsy_log(float pan, float tilt, float roll);
...
protected:
    ...
    void WritePrioritisedBlock(const void *pBuffer, uint16_t size,
                               bool is_critical);
    bool ForcedNoLog() const { return _params.force_no_log; }
