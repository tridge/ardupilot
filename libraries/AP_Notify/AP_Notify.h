...
public:
    ...
    struct notify_flags_and_values_type {
        ...
        bool arming;              // true when vehicle is arming
        bool low_servo_voltage;   // true when servo voltage is low
        bool no_RC_in;            // true when RC is not coming in
        bool ekf_nav_good;        // true when EKF is ready
    };
    ...
    void set_rgb_led_on_off(const bool state);
    bool get_rgb_led_on_off() const;
