...
#define AP_MOTORS_THST_HOVER_MAX        0.8f // maximum possible hover throttle
...
#define AP_MOTORS_ROLL_PITCH_LAG_DEFAULT  0.0f
#define AP_MOTORS_MODIFIED_ARM_SPIN     0.28f   //Not changing param because arm spin shouldn't be above min spin and there are ratios between the two that affect throttle behavior
...
public:
    ...
    // set_roll_pitch_lag - set the advance or delay angle for the roll/pitch commands
    void                set_roll_pitch_lag(float lag_rad) {_roll_pitch_lag_rad.set(lag_rad);}
