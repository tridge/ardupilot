...
public:
    ...
    void                set_auto_motor_offsets(bool en){ _auto_motor_offsets = en; }
    bool                get_auto_motor_offsets(){ return _auto_motor_offsets; }
    void                servo_test_set_mode(uint8_t mode){ _servo_test_mode = mode; }

    void                spoolup_complete(bool complete){ _spoolup_complete = complete; }
    bool                spoolup_complete() const { return _spoolup_complete; }

    float               get_servo_rpm_comp() const {  return _servo_rpm_comp; }

    void                set_aft_rotor_RPM(float RPM)  {  _aft_rotor_RPM = RPM; }
    void                enable_rpm_comp(bool en){ _enable_rpm_compensation = en;}

    float               get_fwd_rotor_thr() {return _fwd_thrust_out;}
    float               get_aft_rotor_thr() {return _aft_thrust_out;}
