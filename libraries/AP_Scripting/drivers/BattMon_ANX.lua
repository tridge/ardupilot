...
local function parse_volt_frame(payload)
   ...
   battery:handle_scripting(BATT_ANX_INDEX:get(), state)
end
...
local function process_frames(msg_type_id)
   ...
   --[[
   This is only needed when playback is done without correct DLC set
   local dlc_correction = 0
   for j = 0, 8 do
      local crc2 = crc_ANX(string.sub(payload, 1, #payload-j))
      if crc == crc2 then
         dlc_correction = j
         payload = string.sub(payload, 1, #payload-dlc_correction)
         gcs:send_text(MAV_SEVERITY.INFO, string.format("dlcofs=%d", dlc_correction))
      end
   end
   --]]
