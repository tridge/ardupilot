        ...
        if (uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_PPP:
                    ...
                    break;
#endif
                case SerialProtocol_ViewPro:
                    // Note baudrate is hardcoded to 115200
                    state[i].baud.set_and_default(AP_SERIALMANAGER_VIEWPRO_BAUD / 1000);   // update baud param in case user looks at it
                    uart->begin(map_baudrate(state[i].baud),
                                         AP_SERIALMANAGER_VIEWPRO_BUFSIZE_RX,
                                         AP_SERIALMANAGER_VIEWPRO_BUFSIZE_TX);
                    break;
                default:
                    uart->begin(state[i].baudrate());
            }
