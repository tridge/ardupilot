...
void AP_Terrain::clear_unused_tiles(Location loc){
    if (!clear_terrain_complete && (options.get() & uint16_t(Options::DeleteUnused)) ){

		auto *d = AP::FS().opendir(HAL_BOARD_TERRAIN_DIRECTORY);

		EXPECT_DELAY_MS(3000);
		if (d == nullptr) {
			gcs().send_text(MAV_SEVERITY_INFO, "TERRAIN: no terrain folder to delete");
			return;
		}
		...
		for (struct dirent *de=AP::FS().readdir(d); de; de=AP::FS().readdir(d)) {
			EXPECT_DELAY_MS(3000);
			bool found_tile_to_delete = true;
			...
			if (strncmp(&de->d_name[length-4], ".DAT", 4)) {
				// doesn't end in .DAT, it is not a valid terrain file
				continue;
			}

			// Compute coordinates of current tile and snap to SW corner
			int32_t lat = abs(loc.lat)/(int32_t)10000000;
			int32_t lon = abs(loc.lng)/(int32_t)10000000;
			if (loc.lat<0){
				lat += 1;
				lat *= -1;
			}
			...
			// Probe the 3x3 array of tiles around the current location
			char terrain_name[13];
			for (int i = -1; i<2; i++) {
				for (int j = -1; j<2; j++) {
					int32_t lat_tmp = lat+i;
					int32_t lon_tmp = lon+j;
					snprintf(terrain_name, 13, "%c%02i%c%03i.DAT",
									lat_tmp<0?'S':'N',
									(int)abs(lat_tmp),
									lon_tmp<0?'W':'E',
									(int)abs(lon_tmp));
					if (strcmp(terrain_name,de->d_name)==0){
						// We have a match. Do not delete.
						found_tile_to_delete = false; 
					}
				}
			}

			if (found_tile_to_delete){
				char *buf = nullptr;
				IGNORE_RETURN(asprintf(&buf, "%s/%s", HAL_BOARD_TERRAIN_DIRECTORY,de->d_name));
				gcs().send_text(MAV_SEVERITY_INFO, "TERRAIN: deleting tile %s", buf);
				AP::FS().unlink(buf);
				free(buf);
			}
		}
