...
public:
    ...
    // return last recorded visual odometry speed
    virtual float last_viso_speed() const { return 0.0f; }
    ...
    // quality of -1 means failed, 0 means unknown, 1 is worst, 100 is best
    virtual void handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, float velErr, uint8_t reset_counter, int8_t quality) = 0;
...
protected:
    ...
    float _last_speed_ms; // Ground speed measurements
