...
bool AP_VisualOdom_IntelT265::healthy() const
{
    uint32_t t_now_ms = AP_HAL::millis();
    bool updating = ((t_now_ms - _last_update_ms) < AP_VISUALODOM_TIMEOUT_MS);
    bool no_recent_reset = (_voxl_reset_counter_last==0) || ((t_now_ms - _last_reset_ms) > 1000U);

    return updating && no_recent_reset;
}
...
// quality of -1 means failed, 0 means unknown, 1 is worst, 100 is best
void AP_VisualOdom_IntelT265::handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, float velErr, uint8_t reset_counter, int8_t quality)
{
    // rotate velocity to align with vehicle
    Vector3f vel_corrected = vel;
    rotate_velocity(vel_corrected);

    float raw_velErr = velErr;
    velErr = constrain_float(velErr, _frontend.get_vel_noise(), 100.0f);
    ...
    if (consume) {
        ...
        AP::ahrs().writeExtNavVelData(vel_corrected, velErr, time_ms, _frontend.get_delay_ms());
    }
...
#if HAL_LOGGING_ENABLED
    Write_VisualVelocity(remote_time_us, time_ms, vel_corrected, raw_velErr, reset_counter, !consume, _quality);
#endif
}
...
bool AP_VisualOdom_IntelT265::align_yaw_to_ahrs(const Vector3f &position, const Quaternion &attitude)
{
    ...
    if (AP::ahrs().using_extnav_for_yaw()) {
        gcs().send_text(MAV_SEVERITY_CRITICAL,"VisOdom MAG align rejected: VIO is enabled");
        return true;
    }
    ...
    if (!AP::ahrs().initialised()
        ...
        gcs().send_text(MAV_SEVERITY_CRITICAL,"VisOdom MAG align rejected: EKF not ready");
        return true;
    }

    align_yaw(position, attitude, AP::ahrs().get_yaw());
    gcs().send_text(MAV_SEVERITY_INFO,"VisOdom MAG align successful");
    return true;
}
