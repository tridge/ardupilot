...
// return true if sensor is basically healthy (we are receiving data)
float AP_VisualOdom::last_viso_speed() const
{
    if (!enabled()) {
        return 0.0f;
    }
    ...
    return _driver->last_viso_speed();
}
...
// quality of -1 means failed, 0 means unknown, 1 is worst, 100 is best
void AP_VisualOdom::handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, float velErr, uint8_t reset_counter, int8_t quality)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return;
    ...
    // call backend
    if (_driver != nullptr) {
        _driver->handle_vision_speed_estimate(remote_time_us, time_ms, vel, velErr, reset_counter, quality);
    }
}
