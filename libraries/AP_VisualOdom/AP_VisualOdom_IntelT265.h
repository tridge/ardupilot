...
public:
    ...
    // return last visual odometry estimate of speed
    float last_viso_speed() const override {return _last_speed_ms;};
    ...
    // quality of -1 means failed, 0 means unknown, 1 is worst, 100 is best
    void handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, float velErr, uint8_t reset_counter, int8_t quality) override;
...
protected:
    ...
    uint32_t _last_reset_ms;   // system time of last reset from sensor (used by health checks)
