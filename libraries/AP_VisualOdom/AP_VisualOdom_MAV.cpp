...
// quality of -1 means failed, 0 means unknown, 1 is worst, 100 is best
void AP_VisualOdom_MAV::handle_vision_speed_estimate(uint64_t remote_time_us, uint32_t time_ms, const Vector3f &vel, float velErr, uint8_t reset_counter, int8_t quality)
{
    // record quality
    _quality = quality;
...
#if HAL_LOGGING_ENABLED
    Write_VisualVelocity(remote_time_us, time_ms, vel, velErr, reset_counter, !consume, _quality);
#endif
}
