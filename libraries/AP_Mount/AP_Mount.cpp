...
void AP_Mount::init()
{
    ...
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        ...
        case Type::Viewpro_AA:
            _backends[instance] = new AP_Mount_Viewpro_AA(*this, _params[instance], instance);
            _num_instances++;
            break;

        case Type::NextVision:
            _backends[instance] = new AP_Mount_NextVision(*this, _params[instance], instance);
            _num_instances++;
            break;

        case Type::GremsyAirpixel:
            _backends[instance] = new AP_Mount_GremsyAirpixel(*this, _params[instance], instance);
            _num_instances++;
            break;

        case Type::GremsyZioVio:
            _backends[instance] = new AP_Mount_GremsyZioVio(*this, _params[instance], instance);
            _num_instances++;
            break;
...
void AP_Mount::update()
{
    ...
    //Following code is AP_Mount implementation of what's in AP_Camera
...
bool AP_Mount::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len)
{
    ...
    for (uint8_t i=0; i<AP_MOUNT_MAX_INSTANCES; i++) {
        if ((get_mount_type(i) != Type::None) && (_backends[i] == nullptr)) {
            strncpy(failure_msg, "Camera Driver Error", failure_msg_len);
            return false;
        }
    }
    ...
    for (uint8_t i=0; i<AP_MOUNT_MAX_INSTANCES; i++) {
        if ((_backends[i] != nullptr) && !_backends[i]->healthy()) {
            strncpy(failure_msg, "Camera Unhealthy", failure_msg_len);
            return false;
        }
    }
...
void AP_Mount::handle_mount_orientation(const mavlink_message_t &msg)
{
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
            _backends[instance]->handle_mount_orientation(msg);
        }
    }
}
...
/*
    Ascent Aerosystems functions
*/

// set_cam_type - sets the type of cam
void AP_Mount::set_cam_type(uint8_t instance, uint8_t cam)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }

    _backends[instance]->set_cam_type(cam);
}

void AP_Mount::set_gcs_type(uint8_t instance, uint8_t gcs)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }

    _backends[instance]->set_gcs_type(gcs);
}

void AP_Mount::set_top_mount(uint8_t instance, uint8_t top)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }

    _backends[instance]->set_top_mount(top);
}

float AP_Mount::get_camera_pan(uint8_t instance) const
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return 0.0;
    }

    return _backends[instance]->get_camera_pan();
}

float AP_Mount::get_zoom_level(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return 1.0;
    }

    return _backends[instance]->get_zoom_level();
}

void AP_Mount::align_roll_pitch_command_to_camera(float &target_roll, float &target_pitch, uint8_t instance) const
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }

    _backends[instance]->align_roll_pitch_command_to_camera(target_roll, target_pitch);
}
...
void AP_Mount::stow(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->stow();
}

bool AP_Mount::supports_ftc(uint8_t instance) const
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return false;
    }
    ...
    // send command to backend
    return _backends[instance]->supports_ftc();
}

void AP_Mount::do_nuc(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->do_nuc();
}

void AP_Mount::payload_take_picture(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->payload_take_picture();
}

void AP_Mount::start_record(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->start_record();
}

void AP_Mount::stop_record(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->stop_record();
}

void AP_Mount::eo_full_screen(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->eo_full_screen();
}

void AP_Mount::ir_full_screen(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->ir_full_screen();
}

void AP_Mount::toggle_heat(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    _backends[instance]->toggle_heat();
}

void AP_Mount::query_sd_info(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return;
    }
    ...
    // send command to backend
    return _backends[instance]->query_sd_info();
}

void AP_Mount::yaw_follow_enable(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
     _backends[instance]->yaw_follow_enable();
}

void AP_Mount::yaw_follow_disable(uint8_t instance)
{
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
     _backends[instance]->yaw_follow_disable();
}

void AP_Mount::eo_ir(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->eo_ir();
}

void AP_Mount::ir_eo(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->ir_eo();
}

void AP_Mount::digital_zoom_1(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->digital_zoom_1();
}

void AP_Mount::digital_zoom_2(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->digital_zoom_2();
}

void AP_Mount::digital_zoom_4(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->digital_zoom_4();
}

void AP_Mount::digital_zoom_8(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->digital_zoom_8();
}

void AP_Mount::digital_zoom_16(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->digital_zoom_16();
}

void AP_Mount::start_tracking(uint8_t instance, int x, int y){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->start_tracking(x, y);
}
...
void AP_Mount::next_color_pallette(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->next_color_pallette();
}

void AP_Mount::toggle_detection(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->toggle_detection();
}

void AP_Mount::set_detection(uint8_t instance, int arg){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->set_detection(arg);
}

void AP_Mount::fusion(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->fusion();
}

void AP_Mount::disable_eo_dzoom(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->disable_eo_dzoom();
}

void AP_Mount::enable_eo_dzoom(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->enable_eo_dzoom();
}

void AP_Mount::set_gimbal_rates(uint8_t instance, float pan_speed, float tilt_speed) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
     _backends[instance]->set_gimbal_rates(pan_speed, tilt_speed);
}

void AP_Mount::set_zoom_rate(uint8_t instance, float zoom_speed) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
     _backends[instance]->set_zoom_rate(zoom_speed);
}

bool AP_Mount::center_camera(uint8_t instance, bool follow_camera) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return false;
    }
    // send command to backend
     return _backends[instance]->center_camera(follow_camera);
}

void AP_Mount::handle_v2_extension(uint8_t instance, const mavlink_message_t &msg) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
    _backends[instance]->handle_v2_extension(msg);
}

void AP_Mount::trigger_menu_control(uint8_t instance, GREMSY_MENU_CONTROL ctrl){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_menu_control(ctrl);
}
void AP_Mount::trigger_display(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_display();
}
void AP_Mount::trigger_power(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_power();
}
void AP_Mount::trigger_af(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_af();
}
void AP_Mount::trigger_c1(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_c1();
}
void AP_Mount::trigger_c2(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_c2();
}
void AP_Mount::trigger_c3(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_c3();
}
void AP_Mount::trigger_mf_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_mf_up();
}
void AP_Mount::trigger_mf_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_mf_down();
}
void AP_Mount::trigger_wb_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_wb_up();
}
void AP_Mount::trigger_wb_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_wb_down();
}
void AP_Mount::trigger_speed_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_speed_up();
}
void AP_Mount::trigger_speed_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_speed_down();
}
void AP_Mount::trigger_aperture_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_aperture_up();
}
void AP_Mount::trigger_aperture_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_aperture_down();
}
void AP_Mount::trigger_iso_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_iso_up();
}
void AP_Mount::trigger_iso_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_iso_down();
}
void AP_Mount::trigger_expcorr_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_expcorr_up();
}
void AP_Mount::trigger_expcorr_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_expcorr_down();
}
void AP_Mount::trigger_zoom_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_zoom_up();
}
void AP_Mount::trigger_zoom_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_zoom_down();
}
void AP_Mount::trigger_focus_up(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_focus_up();
}
void AP_Mount::trigger_focus_down(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->trigger_focus_down();
}
void AP_Mount::start_geotagging(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->start_geotagging();
}
void AP_Mount::stop_geotagging(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->stop_geotagging();
}

void AP_Mount::set_angles(uint8_t instance, float pan_input, float tilt_input) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
     _backends[instance]->set_angles(pan_input, tilt_input);
}

void AP_Mount::handle_data64_extension(uint8_t instance, const mavlink_message_t &msg) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
    _backends[instance]->handle_data64_extension(msg);
}

void AP_Mount::handle_data16_extension(uint8_t instance, const mavlink_message_t &msg) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }
    // send command to backend
    _backends[instance]->handle_data16_extension(msg);
}

void AP_Mount::handle_param_ext_value(uint8_t instance, const mavlink_message_t &msg) {
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

     _backends[instance]->handle_param_ext_value(msg);
}
void AP_Mount::request_payload_info(uint8_t instance){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
    _backends[instance]->request_payload_info();
}

void AP_Mount::update_10hz()
{
    // update each instance
    for (uint8_t instance=0; instance<AP_MOUNT_MAX_INSTANCES; instance++) {
        if (_backends[instance] != nullptr) {
            _backends[instance]->update_10hz();
        }
    }
}

void AP_Mount::set_osd_mode(uint8_t instance, int arg){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->set_osd_mode(arg);
}
void AP_Mount::set_record_source(uint8_t instance, int arg){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->set_record_source(arg);
}
void AP_Mount::set_zoom_mode(uint8_t instance, int arg){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->set_zoom_mode(arg);
}
void AP_Mount::ir_zoom(uint8_t instance, int arg){
    if (instance >= AP_MOUNT_MAX_INSTANCES || _backends[instance] == nullptr) {
        return ;
    }

    // send command to backend
     _backends[instance]->ir_zoom(arg);
}
