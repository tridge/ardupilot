...
void AP_Mount_Backend::set_roi_target(const Location &target_loc)
{
    ...
    _roi_target.lat = target_loc.lat;
    _roi_target.lng = target_loc.lng;

    AP_Terrain *terrain = AP::terrain();

    if (terrain==nullptr || terrain->status()!=AP_Terrain::TerrainStatus::TerrainStatusOK){
        _roi_target.set_alt_cm(0,Location::AltFrame::ABOVE_HOME);
    } else {
        _roi_target.set_alt_cm(0,Location::AltFrame::ABOVE_TERRAIN);
    }
...
void AP_Mount_Backend::clear_roi_target()
{
    ...
    gcs().send_to_active_channels(MAVLINK_MSG_ID_COMMAND_ACK, (const char*)&cmd);
}
...
void AP_Mount_Backend::send_gimbal_device_attitude_status(mavlink_channel_t chan)
{
    ...
    _camera_tilt_angle = ToDeg(att_quat.get_euler_pitch());
    _camera_pan_angle = ToDeg(att_quat.get_euler_yaw());
    _camera_roll_angle = ToDeg(att_quat.get_euler_roll());
...
/*
    Ascent AeroSystems Mount Backend Methods
*/

void AP_Mount_Backend::align_roll_pitch_command_to_camera(float &target_roll, float &target_pitch) {
    Quaternion att_quat;
    if (!get_attitude_quaternion(att_quat)) {
        return;
    }
    ...
    float cos_pan = cosf(yaw_rad);
    float sin_pan = sinf(yaw_rad);

    float temp_roll = target_roll;
    float temp_pitch = target_pitch;

    target_roll =  cos_pan*temp_roll + -sin_pan*temp_pitch;
    target_pitch = sin_pan*temp_roll + cos_pan*temp_pitch;
}
