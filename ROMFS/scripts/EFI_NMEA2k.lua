...
local function parse_engine_param_dynamic(data)
   state.instance, state.oil_press, state.oil_temp, state.temp_K, state.alt_V, state.fuel_rate, state.engine_s, state.cool_press,
      state.fuel_press, state.res1, state.dstat1, state.dstat2, state.eload, state.etorque, state.xxx = string.unpack("<BHHHhhIHHBHHbbB", data)
   if not state.instance then
      return
   end
   ...
   --   gcs:send_named_float('OIL_TEMP', state.oil_temp*0.1)

   -- update fuel used
   local now = millis()
   if state.last_updated_ms == nil then
   ...
   state.fuel_used_gallons = state.fuel_used_gallons + delta_fuel_gallons
   gcs:send_named_float('FUEL_USED', state.fuel_used_gallons)
      ...
      if pgn then
         ...
         elseif pgn == PGN_ENGINE_PARAM_DYNAMIC then
            parse_engine_param_dynamic(data)
             ...
             gcs:send_text(MAV_SEVERITY.INFO, string.format("pgn 0x%04x", pgn))
         end
      end
   end
