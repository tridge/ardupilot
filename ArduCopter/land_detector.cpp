...
void Copter::update_land_detector()
{
    ...
    if (!motors->armed()) {
        ...
        //vehicle is high enough above ground
        if(copter.flightmode->get_alt_above_ground_cm() >= ((int32_t)2*(int32_t)g2.land_alt_low)) {
            set_land_complete(false);
        }
...
void Copter::update_throttle_mix()
{
    ...
    if (flightmode->has_manual_throttle()) {
        ...
        // check if landing
        const bool landing_phase = flightmode->is_landing();

        if (((large_angle_request || force_flying) && !landing_phase) || large_angle_error || accel_moving || descent_not_demanded) {
