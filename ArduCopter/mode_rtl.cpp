...
void ModeRTL::climb_return_run()
{
    ...
    if (_state==SubMode::INITIAL_CLIMB && !copter.failsafe.radio){
        if ((g.throttle_behavior & THR_BEHAVE_LOW_THR_CANCELS_RTL_CLIMB) && channel_throttle->norm_input_dz()<-0.99f){
            // Cancel climb and recompute path using the current altitude
            _state=SubMode::INITIAL_CLIMB;
            _state_complete = true;
            build_path(true);
            return;
...
void ModeRTL::descent_run()
{
    ...
    if (!copter.failsafe.radio) {
        if ((g.throttle_behavior & THR_BEHAVE_HIGH_THROTTLE_CANCELS_LAND) != 0 && copter.rc_throttle_control_in_filter.get() > LAND_CANCEL_TRIGGER_THR  && !copter.ap.cam_function_button_pressed){
...
void ModeRTL::land_run(bool disarm_on_land)
{
    ...
    if (disarm_on_land && copter.ap.land_complete && motors->get_spool_state() == AP_Motors::SpoolState::GROUND_IDLE && copter.flightmode->get_alt_above_ground_cm() <= (int32_t)g2.land_alt_low) {
...
void ModeRTL::build_path(bool no_climb)
{
    ...
    compute_return_target(no_climb);
...
void ModeRTL::compute_return_target(bool no_climb)
{
    ...
    if (alt_type == ReturnTargetAltType::RANGEFINDER) {
        if (copter.get_rangefinder_height_interpolated_cm(curr_alt)) {
           ...
           if (no_climb){
                rtl_path.return_target.set_alt_cm(MAX(curr_alt, RTL_ALT_MIN), Location::AltFrame::ABOVE_TERRAIN);
            } else {
    ...
    if (no_climb){
        target_alt = MAX(MAX(target_alt, curr_alt),RTL_ALT_MIN);
    } else {
        // increase target to maximum of current altitude + climb_min and rtl altitude
