...
bool Copter::set_target_angle_and_throttle(float roll_deg, float pitch_deg, float yaw_deg, float throttle)
{
    // exit if vehicle is not in Guided mode or Auto-Guided mode
    if (!flightmode->in_guided_mode()) {
        return false;
    ...
    mode_guided.set_angle(q, Vector3f{}, throttle, true);
    return true;
}
#endif
...
#endif
    Log_Write_Vehicle_State(motors->get_hover_RPM(),motors->get_servo_rpm_comp());
}
