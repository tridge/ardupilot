...
# include USERHOOK_VARIABLES
    ...
    typedef union {
        struct {
            ...
            uint8_t cam_function_button_pressed : 1; // 29  // camera button is held
        };
        uint32_t value;
    } ap_t;
    ...
    bool set_target_angle_and_throttle(float roll_deg, float pitch_deg, float yaw_deg, float throttle) override;
#endif
#if MODE_CIRCLE_ENABLED == ENABLED
    ...
    void log_init(void);
    void Log_Write_Vehicle_State(float hov_rpm, float servo_rpm_comp);
    ...
    void Decode_Buttons();
    void Detect_Buttons();
    void handle_ftc_mode();
    void Killswitch();
    void topple_sense();
    void servo_voltage_watcher();
    void handle_agc_message(const mavlink_command_int_t &packet);
    void handle_microhard(const mavlink_command_int_t &packet);
    void payload_power(bool on);
