...
MAV_RESULT GCS_MAVLINK_Copter::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    ...
    case MAV_CMD_ASCENT_RID_GCS: {
        copter.opendroneid.pilot_declared_emergency_from_gcs(packet.x != 0);
        return MAV_RESULT_ACCEPTED;
    }
#endif  
    case MAV_CMD_ASCENT_PAYLOAD: {
        copter.handle_agc_message(packet);
        return MAV_RESULT_ACCEPTED;
    }
    case MAV_CMD_ASCENT_MICROHARD: {
        copter.handle_microhard(packet);
        return MAV_RESULT_ACCEPTED;
    }
...
void GCS_MAVLINK_Copter::handle_message(const mavlink_message_t &msg)
{
    ...
    case MAVLINK_MSG_ID_V2_EXTENSION:
        copter.camera_mount.handle_v2_extension(msg);
        break;

    case MAVLINK_MSG_ID_DATA64:
        copter.camera_mount.handle_data64_extension(msg);
        break;

    case MAVLINK_MSG_ID_DATA16:
        copter.camera_mount.handle_data16_extension(msg);
        break;

    case MAVLINK_MSG_ID_PARAM_EXT_VALUE:
        //gcs().send_text(MAV_SEVERITY_INFO, "GCS sent PARAM_EXT_VALUE");
        copter.camera_mount.handle_param_ext_value(msg);
        break;
