...
struct PACKED log_vehicle_state {
    ...
    uint64_t time_us;
    float rpm_hover;
    float servo_rpm_comp;
};


void Copter::Log_Write_Vehicle_State(float hover_rpm, float servo_rpm_comp)
{
    struct log_vehicle_state vec_state_pkt = {
        LOG_PACKET_HEADER_INIT(LOG_DATA_VEC_STATE),
        time_us      : AP_HAL::micros64(),
        rpm_hover    : hover_rpm,
        servo_rpm_comp: servo_rpm_comp
    };
...
// @Field: Value: Value
    ...
    { LOG_DATA_VEC_STATE, sizeof(log_vehicle_state),
      "RPMH",  "Qff",         "TimeUS,H_RPM,SrvCmp", "s--", "F--" },
