...
bool AP_Arming_Copter::run_pre_arm_checks(bool display_failure)
{
    ...
    // Check that the camera feedback is good
    AP_Mount *mount = AP_Mount::get_singleton();
    if (mount == nullptr){
        check_failed(display_failure, "Camera Mount driver not present");
        return false;
    }
...
bool AP_Arming_Copter::gps_checks(bool display_failure)
{
    ...
    if (mode_requires_gps || (checks_to_perform == ARMING_CHECK_ALL || checks_to_perform & ARMING_CHECK_GPS)) {
...
bool AP_Arming_Copter::mandatory_gps_checks(bool display_failure)
{
    ...
    if (mode_requires_gps) {
        if (!copter.position_ok()) {
            ...
            check_failed(display_failure, "GPS Not Ready, Wait for Green Lights");
            return false;
        }
    } else {
        if (fence_requires_gps) {
            if (!copter.position_ok()) {
                ...
                check_failed(display_failure, "Fence enabled, GPS Not Ready");
                return false;
            }
        } else {
            if ( (checks_to_perform == ARMING_CHECK_ALL || checks_to_perform & ARMING_CHECK_GPS) && !copter.position_ok() ) {
                check_failed(display_failure, "GPS Not Ready, Wait for Green Lights");
                return false;
            }
            // return true if GPS is not required and check is not enforced
            return true;
        }
    }
