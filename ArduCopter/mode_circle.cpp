...
void ModeCircle::run()
{
    static bool newRadiusFlag = false;
    ...
    if (!copter.failsafe.radio && copter.circle_nav->pilot_control_enabled()) {
        ...
        if (!is_equal(radius_current, radius_new)) {
            ...
            if(messageCounter == 100) {
                messageCounter = 0;
                copter.circle_nav->set_radius_cm_param(radius_current);
                gcs().send_parameter_value("CIRCLE_RADIUS", AP_PARAM_FLOAT, copter.circle_nav->get_radius());
            }
        }
        else if(newRadiusFlag){ //This is a way to update the ui but not have it spam. Only checks when current and new DO equal eachother i.e. when pilot releases stick. Flag only goes high if pilot changes radius
            newRadiusFlag = false;
            messageCounter = 0;
            copter.circle_nav->set_radius_cm_param(radius_current);
            gcs().send_parameter_value("CIRCLE_RADIUS", AP_PARAM_FLOAT, copter.circle_nav->get_radius());
